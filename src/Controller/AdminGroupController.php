...
class AdminGroupController extends AbstractController
{
    ...
    public function verify(
        ...
        $isCorrectPassword = $this->passwordEncoder->isPasswordValid(
            $user,
            $data["password"]
        );

        return $this->json(
            ["code" => 200, "isCorrectPassword" => $isCorrectPassword],
            200
        );
    }
    ...
    public function deleteGroupe(Request $request)
    {
        ...
        $groupe = new Groupe();
        $groupe = $em->getRepository(Groupe::class)->findOneBy(["id" => $id]);
    ...
    #[
        Route(
            "/admin/groups/assignGroupAdmin",
            name: "admin.group.assignGroupAdmin"
        )
    ]
    public function assignGroupAdmin(
        Request $request,
        EntityManagerInterface $manager,
        ...
        $admin = $this->getChosenAdmin(
            $this->getAvailableGroupAdmin($users),
            $data["email"]
        );
